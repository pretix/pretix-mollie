...
def refresh_mollie_token(event, conditional=False):
    ...
    if conditional:
        ...
        if (
            event.settings.payment_mollie_expires
            and float(event.settings.payment_mollie_expires) - time.time() > 60
        ):
    ...
    try:
        resp = requests.post(
            ...
            data={
                ...
                "redirect_uri": build_absolute_uri(
                    "plugins:pretix_mollie:oauth.return"
                ),
            },
        )
    except Exception:
        ...
        if resp.status_code == 200:
            ...
            for ev in Event_SettingsStore.objects.filter(
                ...
                ev.object.settings.payment_mollie_expires = (
